    ...
    for root, dirs, files in os.walk(path_):
        for name in files:
            if name.endswith(( ".json" )):
                ...
                try:
                    doi_ = get_doi(jfile)
                    cits, url_ =  get_number_citations( doi_ )
                    record = [ doi_, str(cits) ]
                    fo.write( " ".join(record) + "\n" )
                except TypeError:
                    print ("TypeError: ", jfile, doi_)
...
if __name__ == "__main__":
    ...
    o_path = "../db_text/aps_doi_citations.txt"
...
#    
#    my_titles = ["Statistical contact potentials in protein coarse-grained modeling: from pair to multi-body potentials"]
#    dois_ = ["10.1103/PhysRevLett.42.1596", "10.1103/PhysRevLett.4.431"]
#    for doi_ in dois_:
#        print (my_paper ) 
